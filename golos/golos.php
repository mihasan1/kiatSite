...
if ($_GET['v']) {
    ...
    $jumpurl = GetHttpPage("http://www.serverjump.com/jump.aspx?jumpid=" . $jumpid . "&lang=" & $lang);
                ...
                if (count($arr) > 0) {
                    ...
                    for ($myt = 0; $myt < count($arr); $myt++) {
                        ...
                        if (count($arr2) > 0) {
                            $lang = $arr2[0];
                            $jid = $arr2[1];
                            $currpageurl = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] . "?u=" . $urlid . "&l=" . $lang . "&s=" . $serverid . "&j=" . $jid;
                            for ($urlindex = 2; $urlindex < count($arr2); $urlindex++) {
                                echo "<url><loc><![CDATA[" . $currpageurl . "&k=" . $arr2[$urlindex] . "]]></loc></url>";
                            }
                        }
                    }
                    echo '</urlset>';
