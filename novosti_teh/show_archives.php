...
if(!isset($template) or $template == "" or strtolower($template) == "default"){ require_once("$cutepath/data/Default.tpl"); }
    ...
    else{ die("Ошибка!<br>Шаблон <b>$template</b> не существует! Проверьте правильность написания!"); }
}

// Prepare requested categories
if(eregi("[a-z]", $category)){
    die("<b>Ошибка!</b><br>CuteNews обнаружил, что Вы используете \$category = \"".htmlspecialchars($category)."\"; но Вы должны называть их в соответствии с <b>ID</b>, а не по имени!<br>Например:<br><blockquote>&lt;?PHP<br>\$category = \"1\";<br>include(\"path/to/show_archives.php\");<br>?&gt;</blockquote>");
}
...
if($subaction == "" or !isset($subaction)){
        ...
        if(!$handle = opendir("$cutepath/data/archives")){ die("<center>Невозможно открыть папку $cutepath/data/archives "); }
        ...
        if(is_array($arch_arr)){
            ...
            foreach($arch_arr as $arch_file){
                ...
                echo"<a href=\"$PHP_SELF?archive=$arch_file&subaction=list-archive&$user_query\">".langdate("d M Y",$first_timestamp) ." - ". langdate("d M Y",$last_timestamp).", (<b>$count</b>)</a><br />";
            }
        }
}
else{
